...
export class PlayerCharacter {
  ...
  async _loadModels() {
    ...
    try {
      ...
      if (skinObject.animations && skinObject.animations.length > 0) {
        console.log(`[PlayerCharacter] ${skinObject.animations.length} animations trouvées dans le modèle principal :`, 
      ...
      this.#model.traverse(child => {
        if (child.isMesh) {
          ...
          if (child.material) {
            ...
            mats.forEach(m => {
              console.log(`  - Material: ${m.name}, Map: ${m.map ? 'Yes' : 'No'}`);
              m.roughness = 0.7;
              m.metalness = 0.3;
            });
          }
      ...
      const [idleFBX, walkFBX, jumpFBX] = await Promise.all([
        loader.loadAsync(PlayerCharacter.IDLE_PATH).catch(() => ({ animations: [] })),
        loader.loadAsync(PlayerCharacter.WALK_PATH).catch(() => ({ animations: [] })),
        loader.loadAsync(PlayerCharacter.JUMP_PATH).catch(() => ({ animations: [] }))
      ]);

      if (idleFBX.animations.length > 0) this.#animController.addAction('idle', idleFBX.animations[0]);
      if (walkFBX.animations.length > 0) this.#animController.addAction('walk', walkFBX.animations[0]);
      if (jumpFBX.animations.length > 0) this.#animController.addAction('jump', jumpFBX.animations[0]);
      ...
      console.error("[PlayerCharacter] Erreur critique chargement FBX :", error);
    }
  }
  ...
  _computeEyeHeight() {
    ...
    if (headBone) {
      ...
      console.log(`[PlayerCharacter] Hauteur yeux : ${this.#eyeHeight.toFixed(2)}m`);
    } else {
  ...
  update(deltaTime) {
    ...
    this.#physics.update(this.#position, deltaTime);
  ...
  _handleMouseLook() {
    ...
    this.#pitch = Math.max(-PlayerCharacter.PITCH_LIMIT, Math.min(PlayerCharacter.PITCH_LIMIT, this.#pitch));
  ...
  _handleMovement(deltaTime) {
    ...
    const currentSpeed = isRunning ? PlayerCharacter.RUN_SPEED : PlayerCharacter.MOVE_SPEED;
  ...
  _handleJump() {
    if (this.#input.jump) {
      if (this.#physics.requestJump()) {
        this.#animController.playJump();
      }
    }
  }
